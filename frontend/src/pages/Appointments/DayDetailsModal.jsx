...
import { X, Search, User, Clock, CheckCircle, AlertCircle, XCircle, Edit, DollarSign, Lock, Eye } from 'lucide-react';
...
export default function DayDetailsModal({ isOpen, onClose, date, appointments, readOnly, onStatusUpdate, onEdit }) {
    ...
    const filtered = appointments.filter((apt) => {
        ...
        const matchesSearch = !search
            ...
            || apt.sessionName?.toLowerCase().includes(searchLower);
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-2">
                    {filtered.length === 0 ? (
                        ...
                        filtered.map((apt) => {
                            ...
                            const chipClass = apt.status === 'completed' || apt.status === 'auto_completed'
                            ...
                            return (
                                <div key={apt.id} className="flex items-center justify-between p-3 bg-slate-800/50 border border-white/5 rounded-xl hover:bg-slate-800 transition-all group">
                                    <div className="flex items-center gap-3">
                                        <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-sm font-bold ${statusClass}`}>
                                            <Clock size={16} />
                                        </div>
                                        <div>
                                            ...
                                            <div className="text-xs text-slate-400 flex items-center gap-1 mt-0.5">
                                                <User size={10} />
                                                {person.name || t('appointments.member', 'Member')}
                                                {person.phone && <span className="opacity-50">- {person.phone}</span>}
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-1">
                                        {canComplete && (
                                            <button
                                                onClick={(event) => onStatusUpdate(event, apt, 'completed')}
                                        ...
                                        {canMarkMissed && (
                                            <>
                                                <button
                                                    onClick={(event) => onStatusUpdate(event, apt, 'no_show')}
                                                ...
                                                <button
                                                    onClick={(event) => onStatusUpdate(event, apt, 'cancelled')}
                                        ...
                                        {!readOnly && apt.status === 'completed' && apt.coachId && apt.financialRecord?.status !== 'PAID' && (
                                            <button
                                                onClick={(event) => onStatusUpdate(event, apt, 'settle')}
